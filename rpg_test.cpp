...
typedef struct _Character {
    ...
    unsigned int gen_tick, leg_tick = 0, atk_tick = 0, dash_tick;
}Character;
...
void ControlCharacter(unsigned int sistem_tick) {
	...
	if (character.atk_m[0]) {
		...
		if (GetAsyncKeyState(0x58) & 0x8000 && character.dash_tick + 1200 <= tick) {	//dash
			character.x_accel = character.direction * 8 - 4;
			character.dash_tick = tick;
		}
	}
	...
	if (character.x_accel > 1)
		DrawSprite(character.x - 3, character.y, 1, 3, "===");
	if (character.x_accel < -1)
		DrawSprite(character.x + 5, character.y, 1, 3, "===");
...
void MotionControl(short *x, short *y, float *x_accel, float *y_accel, short size_x, short size_y, float *flyTime) {	// Object motion control
	...
	*x += floor(x_value + 0.5); *y += floor(y_value + 0.5);
	...
	if (*x_accel > 0) *x_accel -= 0.5; if (*x_accel < 0) *x_accel += 0.5;
